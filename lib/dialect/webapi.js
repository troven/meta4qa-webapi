...
module.exports = function(learn, config) {
    ...
    learn.given(["I use basic authentication as $principal", "I login as $principal"], function(identity, done) {
        assert(config.agents, "Missing user agents");
        var agent = config.agents[identity];
        assert(agent, "Missing user agent: "+identity);
...
//console.log("JSON FILE: %s ->%s", mime, file);
    ...
    return {
        annotations: function(dialect, annotations, scope) {
            _.extend(scope, { stopwatch: {}, targets: {}, target:{}, request: { qs: {}, headers: {} }, response: {} })

            if (annotations.target) {
                scope.target = scope.targets[annotations.target] || config.targets[annotations.target];
            }
            if (annotations.agent) {
                scope.agent = scope.agents[annotations.agent] || config.agents[annotations.agent];
            }
            if (scope.target) {

...
var WebAPI = module.exports = function(learn, config) {
    ...
    learn.given(["I use a $CERT client certificate", "I use an $CERT client certificate"], function(certName, done) {
        ...
        var cert_dir = this.paths.files || this.files;
...
// debug("HTTP header %s => (%s) %s", name, varname, value);
    ...
    learn.when(["I upload $file"], function(filename, done) {
        assert(filename, "Missing agents");
        assert(this.paths.files, "Missing {files} path");
        var file = helps.files.root(this, filename);
        assert(helps.files.exists(file), "Missing file: "+file);
        this.request.formData = this.request.formData || {};
        this.request.formData.file = fs.createReadStream(file);
        done && done();
    });

...
var debug = require('debug')("meta4qa:dialect:webapi");
...
var WebAPI = module.exports = function (learn, config) {
	...
	learn.given(["I use basic authentication as $agent", "I login as $agent"], function (agent, done) {
        ...
        debug("authorised: %s -> %s", this.credentials.username, this.request.headers.Authorization);
		done && done();
	});
	...
	learn.given(["I use OAuth2", "I use oauth", "I use OAuth2 credentials", "I use oauth credentials"], function (done) {
        ...
        var oauth = _.isObject(this.agent.oauth)?this.agent.oauth:this.agent;
console.log("OAUTH: %j -> %j", this.agent, oauth);
    ...
    learn.given(["I set oauth token from $var", "I set oauth access_token from $var"], function (varname, done) {
        assert(this.agent, "Missing agents");
        var token = helps.vars.find(this, varname);
        assert(token != undefined, "Variable for token " + varname + " is undefined");

        assert(token, "Missing token");
	this.agent.oauth = this.agent.oauth || {};
        helps.vars.set(this.agent.oauth, "access_token", token);
        done && done();
    });
    ...
    learn.given(["I set form $name to $value"], function (name, value, done) {
        helps.webapi.setFormField(this, name, value);
        debug("set form: %s -> %j ", name, value);
        done && done();
    });
	...
	learn.when(["I GET $resource", "I GET from $resource"], function (resource, done) {
...
//        cmd.json = this.request.headers['Content-Type'] == "application/json" || false;
	...
	learn.when(["I GET JSON from $resource", "I GET JSON $resource"], function (resource, done) {
		assert(resource, "Missing GET resource");
		assert(done, "Missing callback");
		assert(this.request, "Missing request");
        assert(this.target, "Missing @target");
    ...
    learn.when(["I download $resource to $path $file"], function (resource, _path, filename, done) {
        assert(resource, "Missing GET resource");
        assert(_path, "Missing $path");
        assert(filename, "Missing $varname");
        assert(this.request, "Missing request");
        assert(this.target, "Missing @target");

        var root = this.paths[_path] || config.paths[_path] || helps.vars.get(this.vars, _path) || helps.vars.get(this, _path);
        var file = helps.files.path(root, filename);
        var basedir = path.dirname(file);
        assert(!helps.files.exists(file), "file already exists: " + file);

        this.request.followRedirect = true;
        var cmd = http.command("GET", resource, this.request, this.target);
        helps.files.mkdirp(basedir);

        request(cmd).pipe(fs.createWriteStream(file)).on('close', function() {
            debug("downloaded: %s", file);
            done && done();
        });
    });
	...
	learn.when("I store body path $path as $name", function (path, name, done) {
		...
		helps.vars.set(this.vars, name, value);
		done && done();
	});
	...
	WebAPI.annotations = function (dialect, annotations, scope) {
        ...
        if (annotations.target) {
            ...
            assert(scope.target, "Missing @target: "+annotations.target);
        } else {
        ...
        // @proxy
        if (annotations.proxy) {
            scope.target.proxy  = _.extend({}, scope.target.proxy, scope.targets[annotations.proxy]);
        } else if (scope.targets.default) {
            assert(scope.target, "Missing scope target");
            assert(scope.targets.default, "Missing scope target");
            scope.target.proxy  = _.extend({}, scope.targets.default.proxy, scope.target.proxy );
        }
	}

	debug("understands REST APIs - v"+pkg.version);

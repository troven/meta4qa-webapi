...
module.exports = function(learn, config) {
    ...
    learn.then(["port $port at $address is open"], function(port, address, done) {
    ...
    learn.then(["port $port is open"], function(port, done) {
        var client = new net.Socket();
    ...
    learn.then(["port $port at $address is closed"], function(port, address, done) {
        try {
            client.connect(port, address, function() {
    ...
    learn.then(["port $port is closed"], function(port, done) {
        try {
            client.connect(port, this.target.hostname, function() {

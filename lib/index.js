...
module.exports = function(learn, config) {
    ...
    learn.given(["I use a $CERT client certificate", "I use an $CERT client certificate"], function(certName, done) {
        assert(config.certificate, "No client certificates");
        var cert = config.certificate[certName];
    ...
    learn.given(["I set $header header to $value","I set header $header = $value"], function(name, value, done) {
        ...
        VERBOSE && console.log("HEADERS: %j", this.request.headers);
        done && done();
    })

    learn.given(["I set parameter $key to $value","I set $key parameter to $value"], function(key, value, done) {
        this.request.qs[key] = value;
        VERBOSE && console.log("QUERY PARAMS: %j", this.request.qs);
        done && done();
    })

    learn.given(/^I set headers to$/, function(headers, done) {
        VERBOSE && console.log("HEADERS: %j ", headers);
        done && done();
    });

    learn.given(["I use basic authentication", "I login", "I authenticate"], function(done) {
        assert(config.agent, "Missing user principals");
        assert(config.agent.default, "Missing default user principal");

        http.authorize(this.request, config.agent.default );
        done && done();
    });

    learn.given("I use basic authentication as $principal", function(identity, done) {
        assert(config.agent, "Missing user principals");
        var principal = config.agent[identity];
        assert(principal, "Missing user principal "+identity);

        http.authorize(this.request, principal);
        done && done();
    });

    learn.given("I use OAuth2", function(done) {
        this.request.headers.Authorization = 'Bearer ' + this.vars.accessToken;
        done && done();
    })

    learn.given("I am debugging", function(done) {
        http.DEBUG = true;
        done && done();
    })
    ...
    learn.given(["I set cookie $cookie to $value","I set cookie $cookie = $value"], function(name, value, done) {
        ...
        console.log("COOKIES: %j", this.request.jar);
        done && done();
    })

    learn.given(["I set timeout to $time"], function(timeInMillis, done) {
...
//console.log("JSON FILE: %s ->%s", mime, file);
    ...
    learn.when("I store body path (.*) as access token", function(path, done) {
        var accessToken = http.findInPath(this.response.body, path);
        assert(accessToken, "Body path "+path+" does not contains an access token");
        this.vars.accessToken = accessToken;
        done && done();
    });
    ...
    return {
        annotations: function(dialect, annotations, scope) {
            ...
            if (scope.target) {
                debug("Use API target: "+ (scope.target.hostname||"explict only"));
            }
        }
    }
};

...
var net = require("net");
var debug = require("debug")("apigeek:helps:webapi");
var ApiGeek = require('apigeek-dialect'), helps = ApiGeek.helpers, files = helps.files, http=helps.http;
...
var webapi = module.exports = {
    ...
    attachFileByType:  function(self, path, file, type, done) {
        assert(self, "Missing self");
        assert(self.request, "Missing self request");
        assert(self.headers, "Missing self headers");
        assert(path, "Missing path");
        assert(file, "Missing file");
